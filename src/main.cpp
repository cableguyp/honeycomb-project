...
#if defined(NDEBUG)
# error "Honeycomb cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb3b9056101b65b1bb9935a667644acd41ed588a04bbe088823af1d46e6c71954");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Honeycomb: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Honeycomb Signed Message:\n";
...
{
    // Honeycomb: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 7500000 blocks, which will occur approximately every 3.2 years
    nSubsidy >>= (nHeight / 7500000); // Honeycomb: 7.5m blocks in ~4.7 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Honeycomb: 1 days
static const int64 nTargetSpacing = 20; // Honeycomb: 20 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Honeycomb: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Honeycomb: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xec8c538c62bcbdcab54bc4797fad95ca4f0aefffa67eafebc46fa09c3ad53570");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN 19/Feb/2018 Florida school shooter had at least 10 rifles";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041a47364587022d4dc73888e706cc2c03b1a4a12a86cd58c56cd489724380a1fcd7c695d32786338524c02af1c87aa9a08a447e4fed2da7978ea4b934502c8bad") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519098192;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085121622;

        if (fTestNet)
        {
            block.nTime    = 1519098163;
            block.nNonce   = 386262332;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe0fb7ab26c747e9d6f9639b2721dad5e9a3f77a1638ca8e9bbb5b3a676df2bc1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xca, 0xb2, 0xdf }; // Honeycomb: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HoneycombMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HoneycombMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HoneycombMiner(CWallet *pwallet)
{
    printf("HoneycombMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("honeycomb-miner");
    ...
    try { loop {
        ...
        printf("Running HoneycombMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HoneycombMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HoneycombMiner, pwallet));
}
